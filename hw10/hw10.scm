(define (how-many-dots s)
  (if (not (pair? s)) 0
    (+ (if(and (not (null? (cdr s))) (not (pair? (cdr s)))) 1 0)
        (how-many-dots (car s))
        (how-many-dots (cdr s))
    )
  )
...
(define (make-exp base exponent)
  (cond ((= exponent 0) 1)
        ((= exponent 1) base)
        ((and (integer? exponent) (number? base)) (expt base exponent))
...
(define (derive-exp exp var)
  (make-product (make-product (exponent exp) (make-exp (base exp) (make-sum(exponent exp) '-1))) (derive (base exp) var))
)
