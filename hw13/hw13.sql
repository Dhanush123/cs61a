...
CREATE TABLE schedule AS
  WITH trips(route, destination, flights, cost) AS (
    SELECT departure || ", " || arrival, arrival, 1, price FROM flights WHERE departure = "SFO" UNION
    SELECT route || ", " || arrival, arrival, flights+1, cost+price FROM trips, flights WHERE flights < 2 AND destination = departure
  )
...
CREATE TABLE calories AS
  SELECT count(*) FROM main_course AS ms, pies AS p WHERE ms.calories + p.calories < 2500;

CREATE TABLE healthiest_meats AS
  SELECT ms.meat, min(ms.calories + p.calories) FROM main_course AS ms, pies AS p GROUP BY ms.meat HAVING max(ms.calories + p.calories) <= 3000;
...
CREATE TABLE shopping_list AS
  WITH short_list(name, ratio) AS (
    SELECT name, min(MSRP/rating) FROM products GROUP BY category
  )
  SELECT sl.name AS item, lp.store AS store FROM short_list AS sl, lowest_prices AS lp WHERE sl.name = lp.item;
