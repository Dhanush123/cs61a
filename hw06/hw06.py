...
def balanced(m):
    ...
    if(is_weight(m)):
        ...
        l, r = sides(m)
        if not (length(l) * total_weight(end(l)) == length(r) * total_weight(end(r))):
            return False
        else:
...
def with_totals(m):
    ...
    if(is_weight(m)):
        return m
    for s in sides(m):
        sidez.append(side(length(s),with_totals(end(s))))
    return tree(total_weight(m), sidez)
############
# Mutation #
############
...
def make_fib():
    ...
    12
    """
    fibs = [0, 0, 1, 1] #0th index is counter
    def fiber():
        fibs[0] += 1
        if fibs[0] == 1:
            return 0
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount):
        ...
        return balance
    def protector(amt, pas):
        nonlocal tries
        if pas == password and tries < 3:
            return withdraw(amt)
        elif tries < 3:
            tries += 1
            false_passes.append(pas)
            return "Incorrect password"
        else:
            return "Your account is locked. Attempts: " + str(false_passes)
    return protector
...
def make_joint(withdraw, old_password, new_password):
    ...
    if(type(err)) == str:
        return err
    def joiner(amt, pas):
        if pas == old_password or pas == new_password:
