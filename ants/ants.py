...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                if insect.container and not insect.ant:
                    if self.ant.container and not self.ant.ant:
                        insect.contain_ant(self.ant)
                        self.ant = insect
                    elif not self.ant.container:
                        insect.contain_ant(self.ant)
                        self.ant = insect

                elif self.ant.container and not self.ant.ant:
                    if insect.container and not insect.ant:
                        self.ant.contain_ant(insect)
                    elif not insect.container:
                        self.ant.contain_ant(insect)
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(insect) == QueenAnt and insect.copies == 0:
                return
            if self.ant is insect:
                if self.ant.container and not insect.container:
                    self.ant.contain_ant(insect)
                elif insect.container and self.ant.container:
                    insect.contain_ant(self.ant)
                    self.ant = insect
                assert self.ant == None or (self.ant.container and not insect.container) or (insect.container and self.ant.container), '{0} is not in {1}'.format(insect, self)
        else:
...
class Ant(Insect):
    ...
    def __init__(self, armor=1, bodyguard=False):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range=0, max_range=0):
        ...
        tracker = 0
        while currentPlace is not hive:
            state = (min_range > 0 and tracker >= min_range) or (max_range > 0 and tracker <= max_range) or (max_range == 0 and min_range == 0)
            if currentPlace.bees and state:
...
class WallAnt(Ant):
    """Wallant has high armor and doesn't make moves"""
    name = 'Wall'
    implemented = True
    food_cost = 4
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.copy_num != 0:
          ...
          cur_pos = self.place.exit
          while cur_pos is not None:
              if not type(cur_pos.ant) == QueenAnt:
                  if place.ant is not None and place.ant not in QueenAnt.doubled_ants:
                      cur_pos.ant.damage *= 2
                      QueenAnt.doubled_ants.append(cur_pos.ant)
                cur_pos = cur_pos.exit
          self.throw_at(self.nearest_bee(colony.hive))
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        self.armor -= amount
        if self.armor == 0 and self.copies == 0:
            bees_win()
        # END Problem 13
